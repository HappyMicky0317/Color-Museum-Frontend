                  ...
                  <article className={stylesSecond.mainGrid}>
                    {arr.map((item, index) => {
                      const color = `rgb(${hexRgb(item.hex, { format: "css" })[0]
                        }, ${hexRgb(item.hex, { format: "css" })[1]}, ${hexRgb(item.hex, { format: "css" })[2]
                        })`;
                      if (
...
const GridAllColors = ({
  ...
  return (
    <div className={stylesSecond.dataTableContainer} ref={refOverflow}>
      ...
      <article className={stylesSecond.mainGrid}>
        {data.map((item, index) => {
          const color = `rgb(${hexRgb(item.hex, { format: "css" })[0]}, ${hexRgb(item.hex, { format: "css" })[1]
